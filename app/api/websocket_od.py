...
class EnhancedAIResponseTracker:
    ...
    async def trigger_enhanced_follow_up_responses(self, session_id: str, peer_reactions: List):
        ...
        try:
            ...
            if not session:
                logger.info(f"⚠️ No autonomous session found for {session_id}")
                return
        ...
        eligible_reactions = [
            ...
            if reaction.should_respond and reaction.engagement_level > 0.8
        ]
        ...
        logger.info(f"🏆 Best peer reaction: {responding_character_id} (engagement: {best_reaction.engagement_level:.2f})")
        ...
        if session.current_speaker:
            logger.info(f"⏸️ Cannot trigger peer response - {session.current_speaker} is currently speaking")
            return
        ...
        async def delayed_peer_response():
            try:
                ...
                await generate_enhanced_ai_response(
                    ...
                    peer_triggered=True,
                    trigger_reaction=best_reaction,
                    context=enhanced_context  # ← PASS ENHANCED CONTEXT
                )
        ...
        logger.info(f"✅ Scheduled FAST peer response from {responding_character_id}")
